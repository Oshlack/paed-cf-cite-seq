...
colnames(design) <- c(levels(Group), "Batch")
design
```

### Test for differences in proportions between Control and CF

Stripchart of cell type proportion, stratified by disease status.
...
#### Control versus disease

First test for differences in proportions between control and all CF samples.
...
There is no statistically significant difference in cell type proportions between the CF samples and the control.
